...
%     Im_segement_start = [rand(texsize/8+filtSize*2,filtSize) Im_segments]% Start
    ...
    imwrite(Im_new,sprintf('smoothed_fwn%i_25contrast.jpg',texID))
end
...
%     textures(6).sf(s).matrix = (grating_45_crop + grating_135_crop)/2;
    ...
    for i = 1:length(textures)
        Im_temp = textures(i).sf(s).matrix;
        Im_temp(Im_temp == 2) = 1;
        Im_new = ((Im_temp-0.5)*wn_contrast) + 0.5;
        imwrite(Im_new,sprintf('%s_%i_50contrast.jpg',texture_name{i},sf(s)));
        figure;
        imshow(Im_new)
    end
end
...
for pattern =  1:1
    ...
    if option == 1
        ...
        Im_temp = double(normalised_image);
        wn_contrast =0.5;
        Im_new = ((Im_temp-0.5)*wn_contrast) + 0.5;
        imwrite(Im_new,sprintf('dot pattern white %i_50contrast.png',pattern));
        imwrite(1-Im_new,sprintf('dot pattern black %i_50contrast.png',pattern));
