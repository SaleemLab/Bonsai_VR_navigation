function [gaborFilterA, gaborFilterB, filtOut, smthFiltOut, peak1, peak2, smod] = makeGaborfilter_5_segments(allImages, sf, gauss_win, x, ori,plotTag)
...
respond = smthFiltOut(20:end-20);
[peak_respond1, peakPos1] = max(respond); % start and end part of the track removed
peak1 = [peakPos1+20 peak_respond1];

if peakPos1<40 %
    if peakPos1+40+5 > length(respond) % If over the range
        [peak_respond2,peakPos2] = max(respond((peakPos1+40-5):end));
    else % If within the range
        [peak_respond2,peakPos2] = max(respond((peakPos1+40-5):(peakPos1+40+5)));
    end
elseif peakPos1>40
    if peakPos1-40-5 < 1 % If over the range
        [peak_respond2,peakPos2] = max(respond((1):(peakPos1-40+5)));
    else
        [peak_respond2,peakPos2] = max(respond((peakPos1-40-5):(peakPos1-40+5)));
    end
...
elseif peakPos1==40
    ...
    [possible_peak1,~] = max(respond((1):(peakPos1-40+5)));
    [possible_peak2,~] = max(respond((peakPos1+40-5):end));
    
    if possible_peak1 > possible_peak2
         [peak_respond2,peakPos2] = max(respond((1):(peakPos1-40+5)));
    else
        [peak_respond2,peakPos2] = max(respond((peakPos1+40-5):end));
    end
...
peak2 = [peakPos2+20 peak_respond2];
